...
##### Equivalent of 'binary scan' command with unsigned integers support #####

##### E.g. binary scan $dta Su header #####


# Reference : http://wiki.tcl.tk/4180
...
proc ::websocket::Disconnect { sock } {
    ...
    Push $sock disconnect "Disconnected from remote end"
		...
		# Removing the 'Server' variable also here...
		# Added by Dinesh on Wed Jan 10 12:23:55 IST 2018
    set ser_varname [namespace current]::Server_$sock
		${log}::info "Variable cleanup of $varname & $ser_varname"
		unset -nocomplain $ser_varname
    unset $varname
}
...
proc ::websocket::close { sock { code 1000 } { reason "" } } {
	...
	#catch {binary format Su $code} msg
	catch {binary format S* $code} msg
...
proc ::websocket::test { srvSock cliSock path { hdrs {} } { qry {} } } {
	...
	catch {::base64::encode [sha1::sha1 -bin $sec]} Client(accept)
    }
...
proc ::websocket::upgrade { sock } {
    variable WS
    variable log
		#lappend ::auto_path /repo/dsivaji/atc/cm8/auto/tools/FREEWARE/build/Linux_2.4.7-10/lib
...
proc ::websocket::send { sock type {msg ""} {final 1}} {
	...
	#puts "SREE : [info vars ::websocket::*]<-"

	set ns_vars [info vars ::websocket::*]
	foreach ns_var $ns_vars {
		#puts "$ns_var : [catch {array exists $ns_var} val]"
		if {[array exists $ns_var]} {
			puts "$ns_var : [array get $ns_var]"
			catch {parray $ns_var} what
			puts =$what=
		}
		#puts "val : $val"
		#puts ->[catch {array get $ns_var} issue]<-
		#puts "issue : ->$issue<-"
	}
    variable WS
    variable log
	...
	if { $mlen < 65536 } {
	    ...
	    catch {append header [binary format S* $mlen]} issue
	} else {
	    #append header [binary format Wu $mlen]
	    catch {append header [binary format W* $mlen]} issue
	}
    }
	...
	catch {append header [binary format I* $mask]} issue
...
proc ::websocket::Receiver { sock } {
	...
	return
		...
		set issue {}
    #catch {binary scan $dta Su header} issue
    catch {bscan $dta Su header} issue
	...
	#catch {binary scan $dta Iu mask} issue
	catch {bscan $dta Iu mask} issue
	...
	switch $opcode {
		...
		if { [string length $Connection(read:msg)] >= 2 } {
		    #catch {binary scan [string range $Connection(read:msg) 0 1] Su reason} issue
		    catch {bscan [string range $Connection(read:msg) 0 1] Su reason} issue
		    set msg [encoding convertfrom utf-8 \
				 [string range $Connection(read:msg) 2 end]]
		    close $sock $reason $msg
		} else {
		    catch {close $sock} issue
		}
		return
	    }
	    9 {
...
proc ::websocket::open { url handler args } {
    ...
    for { set i 0 } { $i < 4 } { incr i } {
        ...
        catch {append OPEN(nonce) [binary format I* [expr {int(rand()*4294967296)}]]} issue
    }
